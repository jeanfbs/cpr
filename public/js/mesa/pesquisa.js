$(document).ready(function() {
	...
	var dataTable = $('#tabela_dados_mesas').DataTable( {
			...
			// Ajax propriedades
			"ajax":{
				"url":pt_br.absolute_url+"/panel-control/mesas/pesquisa",
				"type":"POST"
			},
 ...
 // Array de ids das linhas que irão mostrar os detalhes
    ...
    $('#tabela_dados_mesas tbody').off("click",".view").on( 'click', '.view', function () {
        ...
        if ( row.child.isShown() ) {
           ...
           var codigo = parseInt(tr.children("td:eq(0)").text(),10);
		    ...
		    type: "GET",
		    url : pt_br.absolute_url+"/panel-control/mesas/editar",
		    data : {codigo:codigo},
		    dataType: 'json'
		    }).done(function(res){
...
$(document).off("click",".del").on("click",".del",function(){
	...
	$.ajax({
		type: "GET",
	    url: pt_br.absolute_url+"/panel-control/mesas/deletar",
	    data: {id:id}
    }).done(function(res){
