...
var dataTable = $('#tabela_dados').DataTable( {
		...
		// Ajax propriedades
		"ajax":{
			"url":pt_br.absolute_url+"/panel-control/categorias/pesquisa",
			"type":"POST"
		},
    ...
    $('#tabela_dados tbody').off("click",".view").on( 'click', '.view', function () {
        ...
        if ( row.child.isShown() ) {
           ...
           var codigo = parseInt(tr.children("td:eq(0)").text(),10);
		    ...
		    type: "GET",
		    url : pt_br.absolute_url+"/panel-control/categorias/editar",
		    data : {codigo:codigo},
		    dataType: 'json'
		    }).done(function(res){
...
$('#edicao').bootstrapValidator({
	...
	fields: {
		nome: {
			validators: {
				notEmpty: {
					message: pt_br.msg_erro_categoria
				}
			}
		}
...
$(document).off("click",".del").on("click",".del",function(){
	...
	$.ajax({
		type: "GET",
	    url: pt_br.absolute_url+"/panel-control/categorias/deletar",
	    data: {id:id}
    }).done(function(res){
