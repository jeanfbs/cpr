...
------------------------------------------------------------------*/
$('#cadastro_estoque_bebidas').bootstrapValidator({
	message: '',
	fields: {
		qtd_entrada: {
			validators: {
...
/* ------------------------------------------------------------------ 
|	Preenche o select com os bebidas cadastradas
------------------------------------------------------------------*/
$("#bebidas").getBebidasSelect();
var s2b = $("#bebidas").select2();
...
$("#salvar").on('click', function(event) {
	...
	if($("#cadastro_estoque_bebidas .required").validation())
	{
	...
	var dados = $("#cadastro_estoque_bebidas").serializeArray();
	
	$.ajax({
		type: "POST",
        url : pt_br.absolute_url+"/panel-control/estoque-bebidas/cadastro",
        data : dados
    }).done(function(res){
...
{
	$("#cadastro_estoque_bebidas input").each(function(){
		$(this).val("");
	});
	$("#cadastro_estoque_bebidas textarea").val("");
}
