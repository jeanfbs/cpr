...
|	DataTables Plugin
------------------------------------------------------------------*/
var dataTable = $('#tabela_dados_estoque_bebida').DataTable( {
		...
		// Ajax propriedades
		"ajax":{
			"url":pt_br.absolute_url+"/panel-control/estoque-bebidas/pesquisa",
			"type":"POST",
			"data":function(d){
				d.min = $("#min_b").val();
				d.max = $("#max_b").val();
...
------------------------------------------------------------------*/
    $('#tabela_dados_estoque_bebida tbody').off("click",".view").on( 'click', '.view', function () {
        ...
        if ( row.child.isShown() ) {
           ...
           var codigo = parseInt(tr.children("td:eq(0)").text(),10);
		    ...
		    type: "GET",
		    url : pt_br.absolute_url+"/panel-control/estoque-bebidas/editar",
		    data : {codigo:codigo},
		    dataType: 'json'
		    }).done(function(res){
...
|------------------------------------------------------------------------*/
$('#edicao_estoque_bebida').bootstrapValidator({
	message: '',
	fields: {
		qtd_entrada: {
			validators: {
...
|------------------------------------------------------------------------*/
$("#salvar_edicao_b").off("click").on("click",function(){

	/* valida o formulario para: Campos vazios ou senhas diferentes*/
	if($("#edicao_estoque_bebida .required").validation())
	{
	...
	var dados = $("#edicao_estoque_bebida").serializeArray();
...
$(document).off("click",".del").on("click",".del",function(){
	...
	$.ajax({
		type: "GET",
	    url: pt_br.absolute_url+"/panel-control/estoque-bebidas/deletar",
	    data: {id:id}
    }).done(function(res){
...
{
	$("#edicao_estoque_bebida input").each(function(){
		$(this).val("");
	});
	$("#edicao_estoque_bebida select").val("");
	$("#edicao_estoque_bebida textarea").val("");
}
